...
float amplitudeRatio[] = { mg92b, mg90s, mg92b, mg92b,
...
int period = sizeof(walk) / sizeof(int) / 16;
void shutServos() {
  delay(250);
  for (byte i = 0; i < 16; i++) {
    pwm.setPWM(i, 0, 4096);
...
void loop() {
  ...
  else if (num == 'c') {  //move to calibrate pusture for attaching all the legs
    for (int servonum = 0; servonum < 16; servonum++) {
      pwm.setPWM(pwm_pin[servonum], 0, SERVOMIN + range / 2 + middleShifts[servonum] * rotationDirections[servonum] * rangeRatio);
    }
  } else if (num == 'w') {  //perform a walking pattern
    for (int servonum = 0; servonum < 16; servonum++) {
      pwm.setPWM(pwm_pin[servonum], 0, SERVOMIN + range / 2 + (middleShifts[servonum] + walk[t][servonum]) * rotationDirections[servonum] * rangeRatio);
    }
    delay(10);
